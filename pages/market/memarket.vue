<template>
	<view class="container spell">
				...
				<uni-swipe-action-item v-for="(item,index) in list" :key="index">
					<view class="uitem" @click="goDetail(item)">
						...
						<view class="content" v-if="type==1">
							<view class="conitem">
								<image src="/static/images/icon_rgift.png" mode="aspectFit" style="width: 32rpx;height: 32rpx;margin-right: 13rpx;"></image>
								<view class="text">
									<text class="bold">【收物】：</text>
									<view class="sub u-line-2">{{item.product_name}}</view>
								</view>
							</view>
							<view class="conitem">
								<image src="/static/images/icon_prices.png" mode="aspectFit" style="width: 32rpx;height: 32rpx;margin-right: 13rpx;"></image>
								<view class="text">
									<text class="bold">【收价】：</text>
									<view class="sub u-line-2">{{item.price==''?'价格合理就好':item.price}}</view>
								</view>
							</view>
		...
		<u-loadmore :status="status" bg-color="#f7f8fa" color="#010101" font-size="20" />
...
<script>
	export default{
		...
		methods:{
			...
			loadData(){
				this.$api.getmylist({params:this.type,type:"jishi",page:this.current_page,limit:10}).then((res)=>{
					if(res.code==200){
