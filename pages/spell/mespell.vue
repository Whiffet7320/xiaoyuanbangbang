		...
		<view class="ulist">
			<uni-swipe-action>
				<uni-swipe-action-item v-for="(item,index) in list" :key="index">
					<view class="uitem" @click="goDetail(ptype,item)">
						<view class="hd">
					...
					<template v-slot:right>
						<view class="slotbtn" @click="bindClick('del',index,item.id)">
...
<script>
	export default{
		...
		methods:{
			...
			loadData(){
				if(this.type==1){
					this.$api.getmylist({type:"pinche",page:this.current_page,limit:10}).then((res)=>{
						if(res.code==200){
					...
					this.$api.getmylist({type:"pindan",page:this.current_page,limit:10}).then((res)=>{
						if(res.code==200){
							uni.stopPullDownRefresh();
							this.list = this.reload ? res.data.data : this.list.concat(res.data.data);
							this.list.forEach(ele => {
